...
def get_repo(
    ...
    with FileLock(lock_path):
        if not repo_dir.exists():
            ...
            # Update repo (gracefully fall back to cache on network errors)
            ...
            except (git.GitCommandError, git.exc.GitError, OSError) as e:
                # Network unavailable — use cached version
                import warnings
                warnings.warn(
                    f"Failed to update repo {repo_url}: {e}. "
                    f"Using cached version at {repo_dir}.",
                    RuntimeWarning,
                    stacklevel=2,
                )
            finally:
