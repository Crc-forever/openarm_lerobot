	...
	const unsubscribeCameraViews = onCameraViewsChanged((config) => {
		try {
			...
			floatingKeys.forEach((key, index) => {
				...
				const isSoloAurora =
					key === "aurora_rgb" && floatingKeys.length === 1;
				const xOffset = isSoloAurora ? 0.55 : startX + index * spacing;
				// The handle sits below the 0.6 m panel, so -0.45 places the
				// video itself slightly below eye level instead of above it.
				const yOffset = isSoloAurora ? -0.45 : 0;
